-- Update the RPC function with correct parameter types
DROP FUNCTION IF EXISTS upsert_profile_with_arrays;
...
BEGIN
    INSERT INTO public.profiles (
        id, payment_model, subscription_package, first_name, last_name, phone, city, state,
        date_of_birth, gender, height, weight, chest, waist, hips, medical_conditions,
        injuries, medications, doctor_clearance, fitness_goals, specific_goal, target_date,
        current_activity_level, fitness_level, emergency_contact_name, emergency_contact_phone,
        emergency_contact_relationship, preferred_days, sessions_per_week, preferred_workout_times,
        liability_waiver_accepted, terms_accepted, photo_consent, preferred_payment_method,
        payment_setup_required, payment_setup_status, referral_source, dietary_restrictions,
        onboarding_completed_at, updated_at
    ) VALUES (
        p_id, p_payment_model, p_subscription_package, p_first_name, p_last_name, p_phone, p_city, p_state,
        CASE WHEN p_date_of_birth IS NOT NULL AND p_date_of_birth != '' THEN p_date_of_birth::DATE ELSE NULL END,
        p_gender, p_height, p_weight, p_chest, p_waist, p_hips, p_medical_conditions,
        p_injuries, p_medications, p_doctor_clearance, p_fitness_goals, p_specific_goal,
        CASE WHEN p_target_date IS NOT NULL AND p_target_date != '' THEN p_target_date::DATE ELSE NULL END,
        p_current_activity_level, p_fitness_level, p_emergency_contact_name, p_emergency_contact_phone,
        p_emergency_contact_relationship, p_preferred_days, p_sessions_per_week, p_preferred_workout_times,
        p_liability_waiver_accepted, p_terms_accepted, p_photo_consent, p_preferred_payment_method,
        p_payment_setup_required, p_payment_setup_status, p_referral_source, p_dietary_restrictions,
        CASE WHEN p_onboarding_completed_at IS NOT NULL AND p_onboarding_completed_at != '' THEN p_onboarding_completed_at::TIMESTAMPTZ ELSE NOW() END,
        NOW()
    )
